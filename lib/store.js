...
class SessionStore extends Store {
	...
	set(sid, session, callback) {
		...
		// If we attempt to save, and there isn't a cookie attached to the session,
		// then express-session will throw errors upon retrieving the session from
		// the database. To avoid errors, prevent saving to the database in the
		// first place
		//
		// See: https://github.com/Wingysam/Christmas-Community/issues/6
		if (!('cookie' in sess)) {
			debug('====> session contained no cookie, bailing',sid);
			callback();
			return;
		}
		...
		const old_cookie = session.cookie;
		const restore_cookie_callback = () => {
			session.cookie = old_cookie;
			callback();
		}
		...
		if(session.$commit) {
			session.$dirty = false;
			this.pouch.put(session_to_save,restore_cookie_callback);
		}
		else {
			restore_cookie_callback();
		}
	}
